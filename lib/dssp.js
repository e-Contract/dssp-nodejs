...
DSSP.prototype.sign = function (data, res) {
    ...
    soap.createClient("https://www.e-contract.be/dss-ws/dss?wsdl", function (err, client) {
        client.DigitalSignatureService.DigitalSignatureServicePortImplPort.sign({
            ...
            var serverSecret = new Buffer(response.OptionalOutputs.RequestSecurityTokenResponseCollection.RequestSecurityTokenResponse.Entropy.BinarySecret, "base64");
            var key = psha1(clientSecret, serverSecret);
            console.log("server key size: " + serverSecret.length + " bytes");
            ...
            signature.keyInfoProvider = new SecurityTokenReferenceKeyInfoProvider(tokenIdentifier);
